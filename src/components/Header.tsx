        ...
        <div className="fixed inset-0 z-50 bg-background/98 backdrop-blur-xl flex flex-col items-center pt-24 px-4">
          ...
          <div className="w-full max-w-xl mt-8 space-y-2">
            {results.map((a) => (
              ...
              >
                <span className="text-[10px] font-bold uppercase tracking-[3px] text-accent">{a.categoria}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-1">
              {CATEGORIAS.filter(c => c.slug !== "" && c.slug !== "envio").map((cat) => (
                <Link
                  key={cat.slug}
            ...
            <div className="hidden md:flex items-center gap-3 ml-auto">
              ...
              <Link
                to="/envio"
                className="bg-accent text-accent-foreground text-xs font-bold uppercase tracking-wider px-4 py-2 rounded-full hover:opacity-90 transition-opacity"
              >
                Enviar texto
              </Link>
            </div>
